...
let prospectsProcessed: number = 0;
let currentPage: number = 1;
let pagesProcessed: number = 0;
let currentProspectsList: Element[] = [];
let currentProspectIndex: number = 0;
...
let autoAdjust: boolean = false; // auto-adjust max connections setting
...
const connectToProspectAtIndex = async (): Promise<void> => {
  return new Promise((resolve) => {
    ...
    const firstName = extractFirstName(prospectElement.textContent || "");

    // Find the connect button within this specific prospect
    const connectButton = prospectElement.querySelector<HTMLButtonElement>(
    ...
    setTimeout(() => {
      const modal = document.querySelector<HTMLElement>(".ember-view .send-invite");
      if (modal) {
        const addNoteButton = modal.querySelector<HTMLButtonElement>('button[aria-label^="Add"]');
        if (addNoteButton) {
          ...
          setTimeout(async () => {
            ...
            if (noteTextArea) {
              ...
              // @ts-expect-error - trustedTypes is not in standard Window type
              ...
              if (isLiveMode) {
                ...
                if (cancelButton) {
                  ...
                  await new Promise<void>((resolveInner) => {
                    setTimeout(() => {
                      const dismissButton = modal.querySelector<HTMLButtonElement>(
...
const initializeCurrentPageList = (): void => {
  // Use all selectors together for maximum coverage
  let prospects: Element[] = [
...
const clickMoreButton = (): Promise<void> => {
  return new Promise((resolve) => {
    ...
    let moreElement: Element | null = null;
...
const checkForNewProspects = (previousCount: number): boolean => {
  // Re-scan for prospects
  const newProspects: Element[] = [
    // Search results specific selectors
    ...document.querySelectorAll("div[role=main] div > a"),
    ...document.querySelectorAll(".search-results-container ul li"),
...
const processCurrentPage = async (): Promise<void> => {
...
const processSearchResults = async (): Promise<void> => {
  ...
  const nextPageButton = document.querySelector<HTMLButtonElement>("button[aria-label='Next']");
...
// Function to start the connection process
const startConnectionProcess = async (): Promise<void> => {
